...
@login_required
def sync_posts(request: HttpRequest, workspace_id) -> HttpResponse:
    """Trigger post discovery for all connected accounts in the workspace.

    Returns a small HTMX partial with the result counts so the button can
    show inline feedback without a full page reload.
    """
    import time
    ...
    results = discover_all_posts(limit=25)
    total_new = sum(results.values())

    # If new posts were discovered, immediately fetch their per-post metrics
    # so the user sees stats without waiting for the hourly cron.
    if total_new:
