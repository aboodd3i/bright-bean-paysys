...
DAILY_REPORT_INTERVAL_SECONDS = 86400  # 24 hours


@background(schedule=0)
def send_daily_analytics_report() -> None:
    """Daily cron: compile analytics stats and email the report.

    Registered as a recurring task in ``AnalyticsConfig.ready()`` via
    ``post_migrate`` (same pattern as ``sync_all_account_analytics``).
    Runs once every 24 hours. The recipient and SMTP credentials are
    configured in ``.env``.
    """
    from apps.analytics.report import send_report_email

    send_report_email(days=30)


# ---------------------------------------------------------------------------
# Post discovery: import externally-published posts
# ---------------------------------------------------------------------------

POST_DISCOVERY_INTERVAL_SECONDS = 3600  # hourly


@background(schedule=0)
def sync_platform_posts() -> None:
    """Hourly cron: discover posts published externally on connected accounts.

    Fetches each platform's recent feed and creates ``PlatformPost`` records
    for posts not yet tracked in Brightbean. Once imported, the existing
    analytics sync (``sync_all_account_analytics``) picks them up and fetches
    per-post metrics automatically.
    """
    from apps.analytics.post_discovery import discover_all_posts
