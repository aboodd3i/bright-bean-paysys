...
from .reactions import remove_processing_reaction
...
def process_inbound_event(
    ...
    # --- 4. Normalize ---
    #
    # Mention-only messages (e.g. "@bot" with no additional text) normalize
    # to an empty string.  The LLM cannot receive an empty user message, so
    # we convert this specific case into a small internal conversational
    # prompt.  The final response is always generated by the LLM — never
    # a hardcoded greeting.
    #
    # Punctuation-only messages remain IGNORED.
    from .normalization import clean_slack_text

    cleaned_text = clean_slack_text(event.message_text)
    try:
        request = normalize_inbound_event(event)
    except SlackNormalizationError:
        if not cleaned_text:
            # Mention-only → synthetic prompt for the LLM
            logger.info(
                "Mention-only message: event_id=%s — using internal prompt",
                event_id,
            )
            request = SlackAnalyticsRequest(
                ...
                text=(
                    "The user mentioned the bot without adding a question. "
                    "Respond briefly, greet them, and ask how you can help "
                    "with social-media analytics."
                ),
            )
        else:
            logger.info(
                "Event ignored by normalization: event_id=%s", event_id,
            )
    ...
    # --- 4b. Deterministic simple-message routing (classification only) ---
    # The routing layer classifies messages for internal use but does NOT
    # bypass the LLM.  Every approved conversation message — greetings,
    # help, status, analytics — goes through the LLM which decides
    # whether to respond directly or call an analytics tool.
    route_simple_command(request)  # classification only, no early return
...
def process_inbound_event_task(event_id: str):
    ...
    Wired with a delivery wrapper that removes the 👀 processing
    reaction before posting the final Slack response.
    """
    process_inbound_event(event_id, deliver_response=_deliver_with_reaction_cleanup)


def _deliver_with_reaction_cleanup(
    channel_id: str,
    text: str,
    thread_ts: str = "",
    event=None,
    ...
    """Delivery wrapper that removes the 👀 reaction before posting.

    Removes the processing reaction from the original user message
    (identified by ``event.channel_id`` + ``event.event_ts``) and then
    calls the real ``deliver_slack_response``.

    Reaction removal is best-effort — if it fails, the response is
    still posted.
    """
