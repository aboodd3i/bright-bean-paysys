...
class LinkedInProvider(SocialProvider):
    ...
    def get_recent_posts(self, access_token: str, limit: int = 25) -> list[dict]:
        """Fetch recent posts by the LinkedIn author."""
        ...
        for item in resp.json().get("elements", []):
            ...
            text_parts = item.get("commentaryV2", {}).get("text", {}).get("value", "")
            if not text_parts:
                text_parts = item.get("text", {}).get("text", "")
            created = item.get("createdAt", 0)
            if created:
                try:
                    published_at = datetime.fromtimestamp(created / 1000, tz=UTC)
            ...
            posts.append({
                ...
                "permalink": self._post_urn_to_url(post_urn) if post_urn else "",
                "media_type": "",
            })
        return posts
